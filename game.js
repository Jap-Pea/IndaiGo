...
let enemyFireRate = 0.002 // chance per frame per enemy
let enemyBullets = []
let score = 0
let lives = 3
...
addEventListener('keydown', (e) => {
  ...
  keys.add(e.key)
  if (mode === 'gameover' && e.key === 'Enter') startGame()
})
...
let last = 0
function loop(ts) {
  const dt = (ts - last) / (1000 / 60) // frames at 60fps scale
  last = ts
  update(dt)
  draw()
...
function update(dt) {
  ...
  // player move
  if (keys.has('ArrowLeft')) player.x -= player.speed * dt * 1.0
  if (keys.has('ArrowRight')) player.x += player.speed * dt * 1.0
  ...
  // next wave
  if (enemies.length === 0) {
    enemySpeed += 0.25 // ramp difficulty
...
function draw() {
  ...
  // When not playing, show overlay text
  if (mode !== 'playing') {
...
// initial render shows menu + highscores
